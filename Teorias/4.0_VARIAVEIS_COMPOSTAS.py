...
# lanches[3] = 'Suco' #erro pois tuplas são imutáveis. Não pode adicionar
# ou subtituir variaveis nas tuplas

for contador in range(0, len(lanches)):
    # imprime lanche por lanche - melhor quando precisa saber posição
    print(f'Vou comer {lanches[contador]}')

for comida in lanches:
    # imprime lanche por lanche como exemplo de cima
    print(f'Vou comer {comida}')

for contador in range(0, len(lanches)):
    # saber posição
    print(f'Vou comer {lanches[contador]} na posição {contador}')

for posicao, comida in enumerate(
        lanches):  # enumerate caso precise saber posição
    print(f'Vou comer {comida} na posição {posicao}')

# irá printar lanches em ordem alfabética. Apeas printar. (transformou a
# tupla em lista [])
...
# MAIS DE UMA VARIAVEL DEFINIDA NO MESMO "ESPAÇO". USAR []
# DIFERENTE DAS TUPLAS, LISTAS SÃO MUTAVEIS - PODE-SE FAZER OPERAÇÕES DE
...
# adiciona um elemento na lista, ele será colocada no últioma posição
lista.append(8)
print(lista)

# adiciona um alemento numa posição especifica, no exemplo na posição 2
lista.insert(2, 0)
print(lista)
...
# remove o elemento 2 da lista, se houverem mais que um elemento 2, ele
# remove o primeiro
lista.remove(2)
...
teste.append(27)
gurizada = list()
# colocado uma copia da lista em gurizada, pois foi usado [:]
gurizada.append(teste[:])
print(gurizada)
...
gurizada = [['Pedro', 20], ['Joao', 19], ['Maria', 21]]
...
pessoas = {'nome': 'Paulo', 'sexo': 'M',
           'idade': 22}  # declarando um dicionario
# nome é uma chave, paulo é um valor. o conjunto de chaves compoe um item
# (value>keys>items)
...
# quando usar o format, precisa usar aspas duplas " pois esta dentro das
# aspas simples
...
for k, v in pessoas.items():
    # k sera as keys e v os valores. nao precisa de enumarate como nas litas e
    # tuplas
    print(k, v)

# substitui o valor anterior em nome. nao precisa dar append
...
for c in range(0, 3):
    ...
    # nao pode usar [:] para criar uma copia de biblioteca. por isso usar
    # .copy()
...
for e in brasil:
    for k, v in e.items():  # onde k vai ser chaves e v os valores
        # mostra valores e suas rescpecticas chaves
        print(f'O chave {k} tem valor {v}')
